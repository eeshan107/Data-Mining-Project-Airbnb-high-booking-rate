...
# load
new_train_x_data <- readRDS('sub_8_train_x_data.rds')
new_train_y <- read_csv("airbnb_train_y_2023.csv")
new_test_x_data <- readRDS('sub_8_test_x_data.rds')
...
#plot(log10(grid), aucs_rf)
auc_rf <- mean(aucs_rf)
auc_rf

rf_mod_new <- ranger(x = train_dum, y = as.factor(new_train_y$high_booking_rate),
                 mtry=100, num.trees=300,
                 importance="impurity",
                 probability = TRUE)
test_prob_rf <- predict(rf_mod_new, data=test_dum)$predictions[,2]
# Calculate AUC


submit_df <- data.frame(x = test_prob_rf)
write.csv(submit_df, file = "airbnb_test_predictions.csv", row.names = FALSE)
########################



#####Ridge and Lasso#######
glm.out.ridge <- glmnet(inner_train_x, inner_train_y, alpha = 0, family="binomial")
glm.out.lasso <- glmnet(inner_train_x, inner_train_y, alpha = 1, family="binomial")
